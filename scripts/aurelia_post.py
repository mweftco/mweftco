#!/usr/bin/env python3
"""
AURELIA v3.0 - AI Fashion Content Agent with Direct Image URLs
Uses Pollinations.ai direct image URLs (no download needed)
"""
...
OUTFITS = [
    {"title": "The Power Navy Suit", "outfit": "tailored navy blue business suit with white dress shirt, black leather oxford shoes, silver watch", "desc": "Command the room with this sharp navy power suit. Perfect for meetings, presentations, and making a statement.", "tag": "FORMAL"},
    {"title": "Midnight Black Elegance", "outfit": "slim-fit all-black suit with black turtleneck, black Chelsea boots, minimalist silver accessories", "desc": "All black never goes out of style. Sleek, mysterious, and undeniably powerful.", "tag": "FORMAL"},
    {"title": "Casual Luxe Friday", "outfit": "cream oversized blazer, black fitted t-shirt, dark denim jeans, white leather sneakers", "desc": "Relaxed but refined. The perfect balance between comfort and high fashion.", "tag": "SMART CASUAL"},
    {"title": "Desert Beige Statement", "outfit": "beige linen suit with open-collar white shirt, brown leather loafers, gold accent watch", "desc": "Light, breathable, and effortlessly stylish. Beige is the new power color.", "tag": "SUMMER FORMAL"},
    {"title": "Street Royalty", "outfit": "black oversized hoodie under camel wool coat, black cargo pants, high-top white sneakers, chain necklace", "desc": "Streetwear meets luxury. For the modern man who writes his own rules.", "tag": "STREETWEAR"},
    {"title": "The Classic Tuxedo", "outfit": "black peak lapel tuxedo, crisp white pleated shirt, black bow tie, patent leather shoes", "desc": "When the occasion demands perfection. Timeless black-tie elegance.", "tag": "BLACK TIE"},
    {"title": "Monochrome Minimal", "outfit": "grey wool overcoat, light grey turtleneck sweater, charcoal trousers, black leather boots", "desc": "Less is more. Shades of grey create a sophisticated, modern silhouette.", "tag": "MINIMAL"},
    {"title": "Bold in Burgundy", "outfit": "burgundy velvet blazer, black fitted shirt, black tailored trousers, black dress shoes", "desc": "Stand out without shouting. Burgundy adds richness and depth to any wardrobe.", "tag": "STATEMENT"},
    {"title": "Summer Linen Dream", "outfit": "white linen shirt unbuttoned at collar, beige linen trousers, brown leather sandals, sunglasses", "desc": "Beat the heat in style. Linen is the ultimate summer luxury fabric.", "tag": "SUMMER"},
    {"title": "The Modern Sherwani", "outfit": "black embroidered sherwani with gold detailing, matching churidar, black traditional shoes", "desc": "Where tradition meets contemporary fashion. Perfect for weddings and celebrations.", "tag": "ETHNIC"},
    {"title": "Leather & Denim Edge", "outfit": "black leather jacket, white crew neck tee, dark blue raw denim jeans, black combat boots", "desc": "Rebel with a cause. This look is bold, confident, and unapologetic.", "tag": "EDGY"},
    {"title": "Office to Evening", "outfit": "charcoal grey three-piece suit, light blue shirt, burgundy tie, brown brogue shoes", "desc": "From boardroom to bar. One outfit that works from 9 AM to midnight.", "tag": "VERSATILE"},
    {"title": "Resort Luxe", "outfit": "floral print silk shirt, white tailored shorts, leather boat shoes, aviator sunglasses", "desc": "Vacation mode activated. Luxury resort wear for the modern gentleman.", "tag": "RESORT"},
    {"title": "Winter Layer King", "outfit": "navy wool peacoat, grey cashmere scarf, black turtleneck, dark jeans, black leather boots", "desc": "Master the art of layering. Warmth and style in perfect harmony.", "tag": "WINTER"},
    {"title": "Athleisure Elite", "outfit": "black fitted tracksuit with white side stripes, white running shoes, crossbody bag, cap", "desc": "Gym to street in seconds. Because style should never slow you down.", "tag": "ATHLEISURE"},
]


def generate_image_url(model, outfit):
    """Generate AI fashion image URL using Pollinations.ai (free, no API key)"""
    prompt = (
...
def generate_post():
    ...
    # Generate direct image URL (no download needed)
    img_url = generate_image_url(model, outfit)
...
.post-image {{ width:100%; border-radius:12px; margin-bottom:20px; display:block; }}
...
<div class="container">
  ...
  <div class="post-card">
    ...
    <img src="{img_url}" alt="{outfit['title']}" class="post-image" loading="lazy">
...
<div class="footer">M&WEFTCO // CRAFTED FOR THE MODERN MAN // AURELIA AI FASHION ENGINE</div>
    ...
    update_index(outfit['title'], slug, date_str, img_url)
...
def update_index(title, slug, date_str, img_url):
    ...
    post_link = f'''<a href="posts/{slug}.html" style="display:block; background:rgba(10,22,45,0.95); border:1px solid rgba(0,100,200,0.12); border-radius:10px; overflow:hidden; text-decoration:none; color:inherit; transition:all 0.2s; margin-bottom:12px;" onmouseover="this.style.borderColor='rgba(0,180,255,0.3)'" onmouseout="this.style.borderColor='rgba(0,100,200,0.12)'">
      <div style="height:160px; background-image:url({img_url}); background-size:cover; background-position:center top;"></div>
...
if __name__ == "__main__":
    print("AURELIA v3.0: Generating AI fashion post with direct image URL...")
    title = generate_post()
    if title:
